...
function createMapPopup(point) {
    ...
    popup.className = 'map-node-popup';

    const name = document.createElement('strong');
    name.textContent = String(point?.name || '未命名监测点');

    const device = document.createElement('span');
    device.textContent = `设备编号：${String(point?.sn_code || '--')}`;

    const location = document.createElement('span');
    location.textContent = String(point?.location || '位置待确认');

    popup.append(name, device, location);
    return popup;
}
        ...
        if (result.success) {
            ...
            points.forEach(point => {
                ...
                if (Number.isFinite(latitude) && Number.isFinite(longitude)) {
                    const position = normalizeLatLng(latitude, longitude);
                    latLngs.push([position.lat, position.lng]);
                    const markerName = String(point?.name || '未命名监测点');
                    const markerLocation = String(point?.location || '').trim();
                    const markerLabel = `${markerName}${markerLocation ? `，${markerLocation}` : ''}，点击切换监测场景`;
                    const marker = L.marker([position.lat, position.lng], {
                        ...
                        .addTo(nanjingMap)
                        .bindPopup(createMapPopup(point));
                    ...
                    const markerKey = String(point.sn_code || '').trim();
                    if (markerKey) mapMarkers[markerKey] = marker;
                }
            });
