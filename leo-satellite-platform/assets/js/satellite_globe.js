...
class SatelliteGlobe {
    ...
    animate() {
        ...
        const reduceMotion = this.reducedMotionQuery?.matches === true;

        if (!reduceMotion && this.earth) {
            this.earth.rotation.y += 0.001 * this.animationSpeed;
        }

        if (!reduceMotion && this.clouds) {
        ...
        this.satellites.forEach((satellite) => {
            ...
            if (!reduceMotion) data.time += data.config.speed * this.animationSpeed;
            ...
            if (!reduceMotion && satellite.userData.ring) {
