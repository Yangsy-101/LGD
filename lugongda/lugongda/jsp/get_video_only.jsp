<%@ page import="java.io.*" trimDirectiveWhitespaces="true" session="false" %><%
    ...
    synchronized (application) {
        frameSeq = (Long) application.getAttribute("ram_video_only_seq");
        ...
        frameBytes = (byte[]) application.getAttribute("ram_video_only");
        frameLength = (Integer) application.getAttribute("ram_video_only_length");
        frameTs = (Long) application.getAttribute("ram_video_only_ts");
        frameSeq = (Long) application.getAttribute("ram_video_only_seq");
    }
    ...
    if (frameBytes != null && frameBytes.length > 0) {
        ...
        if (frameLength != null && frameLength.intValue() > 0) {
            length = Math.min(frameLength.intValue(), frameBytes.length);
        }
    ...
    File file = new File("/var/lib/tomcat/webapps/media/video_only/latest.jpg");
    if (!file.exists() || !file.isFile()) {
        response.setStatus(404);
        response.setContentType("text/plain;charset=UTF-8");
        out.print("video_only/latest.jpg not found");
        return;
    }
