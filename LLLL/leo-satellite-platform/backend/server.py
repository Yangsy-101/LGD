...
def parse_satellite_sensor_fields(packet, content, gateway):
    ...
    if sensor_type == 'TH':
        ...
        if not isinstance(sensor_value, dict):
            raise ValueError('TH sensor value must contain tp and rh')
        ...
        if (
            ...
            raise ValueError('TH sensor tp and rh must be numeric')
    ...
    raise ValueError('Satellite sensor type must be F, TH, or WS')
...
def format_satellite_data(record):
    ...
    return {
        ...
        'ch1_raw': build_scaled_raw(record_get(record, 'temperature'), 100),
        'ch2_raw': build_scaled_raw(record_get(record, 'humidity'), 100),
        'ch3_raw': None,
        'ch4_raw': None,
