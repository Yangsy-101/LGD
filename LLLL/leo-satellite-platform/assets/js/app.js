...
function updateLatestDataDisplay(data, previous) {
    ...
    if (valueTemperature) valueTemperature.textContent = formatMetricValue(data.temperature, 2);
...
function renderHistoryTable(rows) {
    ...
    if (records.length === 0) {
        body.innerHTML = '<tr class="history-empty-row"><td colspan="4">当前接入点暂无历史数据</td></tr>';
        return;
    }
