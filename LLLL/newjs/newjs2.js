...
const GATEWAY_CONFIG = {
    host: '192.168.0.233',      // 网关开发板 IP
    port: 10001,                  // 远海场景网关端口
    streamPath: '/stream',       // MJPEG 流路径

    // 帧相关
    frameTimeoutMs: 5000,        // 超时判定断流，触发重连
    maxJpegSizeBytes: 200 * 1024, // 单帧最大 200KB，超出则丢弃
...
class MjpegReceiver {
    constructor(config) {
        ...
        this.canvas = document.getElementById('ocean-stream-canvas');
        this.placeholder = document.getElementById('ocean-camera-placeholder');
        ...
        // ★ 缓存 drawImage 参数，仅在分辨率/尺寸变化时重算
        this._drawCache = { cw: 0, ch: 0, bw: 0, bh: 0, dx: 0, dy: 0, dw: 0, dh: 0 };
        this._canvasVisible = false;  // ★ 缓存可见性，避免每帧读 style.display
    ...
    _scheduleRetry() {
        if (!this.running) return;
        if (this._retry >= GATEWAY_CONFIG.maxRetries) {
            console.error('[MJPEG] 重连次数用尽');
            this.stop(); return;
        }
        const delay = Math.min(
            GATEWAY_CONFIG.retryBaseDelayMs * Math.pow(2, this._retry),
            GATEWAY_CONFIG.retryMaxDelayMs
        );
        this._retry++;
    ...
    async _processStream(response, signal, boundaryStr) {
        ...
        const MAX = GATEWAY_CONFIG.maxJpegSizeBytes;
        ...
        const decodeLoop = (async () => {
            ...
            while (this.running) {
                ...
                try {
                    // ★ ImageDecoder：直接吃 ArrayBuffer，彻底消灭 Blob 分配和 GC 压力
                    let bitmap, bw, bh;
                    if (typeof ImageDecoder !== 'undefined') {
                        ...
                        bitmap = result.image;  // VideoFrame（实现 CanvasImageSource）
                        bw = bitmap.displayWidth;
                        bh = bitmap.displayHeight;
                    } else {
                    ...
                    // ★ 仅在画布或位图尺寸变化时重算绘制参数
                    const dc = this._drawCache;
                    if (dc.cw !== this._cw || dc.ch !== this._ch || dc.bw !== bw || dc.bh !== bh) {
                        dc.cw = this._cw; dc.ch = this._ch;
                        dc.bw = bw; dc.bh = bh;
                        const scale = Math.min(this._cw / bw, this._ch / bh);
                        dc.dw = Math.round(bw * scale);
                        dc.dh = Math.round(bh * scale);
                        dc.dx = Math.round((this._cw - dc.dw) / 2);
                        dc.dy = Math.round((this._ch - dc.dh) / 2);
                    }
                    this.ctx.drawImage(bitmap, dc.dx, dc.dy, dc.dw, dc.dh);
                    bitmap.close();
                    this._showCanvas();
...
// ============================================================
//
// 本文件由 ocean-iot2.html 引入（相对路径 ../../../newjs/forest_fire.js）
// ocean-iot2.html 中通过内联脚本将"开始检测"/"结束检测"按钮绑定到 MjpegReceiver。
//
// MjpegReceiver 只接管视频流接收，传感器图表、抽屉、警报等仍由 forest-fire.js 负责。
//
// 使用:
//   1. 确认 GATEWAY_CONFIG.host / port / streamPath 与网关一致
//   2. 浏览器打开 ocean-iot2.html
//   3. 点击"开始检测"按钮，接收网关 MJPEG 视频流
//   4. 控制台可查看 FPS 统计: receiver.stats
