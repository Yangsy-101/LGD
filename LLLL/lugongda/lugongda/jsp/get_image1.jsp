<%@ page import="java.io.*" trimDirectiveWhitespaces="true" session="false" %><%
    ...
    synchronized (application) {
        frameSeq = (Long) application.getAttribute("video1LatestSeq");
        ...
        frameBytes = (byte[]) application.getAttribute("video1LatestBytes");
        frameLength = (Integer) application.getAttribute("video1LatestLength");
        frameTs = (Long) application.getAttribute("video1LatestTs");
        frameSeq = (Long) application.getAttribute("video1LatestSeq");

        if (frameBytes == null) {
            frameBytes = (byte[]) application.getAttribute("ram_video_1");
            frameLength = frameBytes == null ? null : Integer.valueOf(frameBytes.length);
            frameTs = (Long) application.getAttribute("ram_video_1_ts");
        }
    }
    ...
    File file = new File("/var/lib/tomcat/webapps/media/video1/latest.jpg");
    if (!file.exists() || !file.isFile()) {
        response.setStatus(404);
        response.setContentType("text/plain;charset=UTF-8");
        out.print("video1/latest.jpg not found");
        return;
    }
